...
module typus::airdrop {
    ...
    public fun set_airdrop<TOKEN>(
        ...
        let mut airdrop_info = if (dynamic_field::exists(&typus_airdrop_registry.id, key)) {
        ...
        if (dynamic_field::exists(& airdrop_info.id, TotalValue.to_string())) {
        ...
        if (!dynamic_field::exists(& airdrop_info.id, ClaimedTable.to_string())) {
    ...
    public fun remove_airdrop<TOKEN>(
        ...
        if (dynamic_field::exists(& id, TotalValue.to_string())) {
            let _: u64 = dynamic_field::remove(&mut id, TotalValue.to_string());
        };

        if (dynamic_field::exists(& id, ClaimedTable.to_string())) {
