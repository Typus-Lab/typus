...
module typus::leaderboard {
    ...
    fun test_leaderboard() {
        ...
        activate_leaderboard(&version, &mut registry, key, 0, 1, test_scenario::ctx(&mut scenario));
        activate_leaderboard(&version, &mut registry, key, 0, 10, test_scenario::ctx(&mut scenario));
        ...
        extend_leaderboard(&version, &mut registry, key, id, 5, test_scenario::ctx(&mut scenario));
        ...
        score(&manager_cap, &version, &mut registry, key, @0xBBBB, 15, &clock, test_scenario::ctx(&mut scenario));
        ...
        trim_leaderboard(&version, &mut registry, key, id, true, 0, 10, test_scenario::ctx(&mut scenario));
        get_rankings(&version, &registry, key, id, 10, scenario.sender(), true);
        clock.set_for_testing(8);
        score(&manager_cap, &version, &mut registry, key, @0xCCCC, 25, &clock, test_scenario::ctx(&mut scenario));
